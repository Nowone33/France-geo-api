...
public class GeoImportService {
    ...
    public GeoImportService(WebClient webClient, GeoSecondaryPort geoSecondaryPort, ObjectMapper objectMapper, RabbitTemplate rabbitTemplate) {
            ...
            for(JsonNode node:  departementsOfficiels) {
                ...
                if(geoMap.containsKey(code)){
                    ...
                    depts = ImmutableZoneGeographique.builder()
                            ...
                            .type("DEPARTEMENT")
                            .parentCode(node.get("codeRegion").asText())
                            .geometrie(geometry)
                            .build();
                ...
                response.getFeatures().forEach(feature -> {
                    ...
                    Map<String, Object> geoMap = objectMapper.convertValue(feature.getGeometry(), Map.class);
                     ...
                     ZoneGeographique commune = ImmutableZoneGeographique.builder()
                             ...
                             .code(props.get("code").asText())
                             .type("COMMUNE")
                             .parentCode(codeDepartement)
                             .geometrie(geoMap)
                             .population(props.has("population")?props.get("population").asLong() : 0L)
                             .build();

                    geoSecondaryPort.save(commune);
                });
            }
